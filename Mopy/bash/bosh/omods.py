...
class OmodFile(object):
    ...
    def readConfig(self, conf_path):
        ...
        with open(conf_path.s, u'rb') as omod_config:
            ...
            self.author = decoder(_readNetString(omod_config)) # author
    ...
    def getOmodConfig(name):
        ...
        if configPath.exists():
            with open(configPath.s,u'rb') as ins:
                ...
                config.name = decoder(_readNetString(ins), encoding='utf-8')
                config.vMajor = unpack_int_signed(ins)
                config.vMinor = unpack_int_signed(ins)
                for attr in ('author','email','website','abstract'):
                    setattr(config, attr, decoder(_readNetString(ins), encoding='utf-8'))
