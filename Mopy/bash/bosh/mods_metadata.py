    ...
    def _scan_Python(modInfos, what, progress, detailed=False,
        ...
        for i,modInfo in enumerate(modInfos):
            ...
            if len(modInfo.masterNames) > 0:
                ...
                with ModReader(modInfo.name,path.open(u'rb')) as ins:
                    try:
                        insAtEnd = ins.atEnd
                        insTell = ins.tell
                                ...
                                if doFog and rtype == b'CELL':
                                    ...
                                    while insTell() < nextRecord:
                                        ...
                                        if nextType != b'XCLL':
                                            ...
                                            color,near,far,rotXY,rotZ,fade,clip = ins_unpack(nextSize,b'CELL.XCLL')
...
class NvidiaFogFixer(object):
    ...
    def fix_fog(self, progress,
        ...
        #--Scan/Edit
        with ModReader(self.modInfo.name,path.open(u'rb')) as ins:
            ins_unpack = partial(ins.unpack, __unpacker)
            with path.temp.open(u'wb') as  out:
                def copy(size):
                    buff = ins.read(size)
                    out.write(buff)
                def copyPrev(size):
                ...
                while not ins.atEnd():
                    ...
                    if type == u'GRUP':
                        if header.groupType != 0: #--Ignore sub-groups
                            pass
                        elif header.label not in (u'CELL',u'WRLD'):
